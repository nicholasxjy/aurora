...
local link_ts_v8 = function(bg)
	...
	vim.api.nvim_set_hl(
		...
		{ fg = ct.white, bg = bg, ctermfg = 189, ctermbg = "NONE", bold = (vim.g.aurora_bold == 1) }
	)
	...
	vim.api.nvim_set_hl(0, "@constructor", { fg = ct.blued, ctermfg = 68, bold = (vim.g.aurora_bold == 1) })
	...
	vim.api.nvim_set_hl(
		0,
		"@text.emphasis",
		{ italic = (vim.g.aurora_italic == 1), bold = (vim.g.aurora_bold == true) }
	)
	...
	colorscheme = function()
		...
		local function load_basic()
			...
			vim.api.nvim_set_hl(
				0,
				"CursorLine",
				{ fg = "NONE", bg = bgfl, ctermfg = "NONE", ctermbg = 236, bold = (vim.g.aurora_bold == 1) }
			)
			...
			vim.api.nvim_set_hl(
				0,
				"DiffAdd",
				{ bg = ct.dimgreen, ctermfg = "NONE", ctermbg = 239, bold = (vim.g.aurora_bold == 1) }
			)
			vim.api.nvim_set_hl(
				0,
				"DiffChange",
				{ fg = ct.yellow1, bg = ct.neardark3, ctermfg = "NONE", ctermbg = 236, bold = (vim.g.aurora_bold == 1) }
			)
			vim.api.nvim_set_hl(
				0,
				"DiffDelete",
				{ fg = ct.gray, bg = ct.neardark3, ctermfg = "NONE", ctermbg = 239, strikethrough = true }
			)
			...
			vim.api.nvim_set_hl(
				0,
				"SpellBad",
				{ fg = ct.red5, bg = bg, ctermfg = 130, ctermbg = "NONE", sp = ct.red5, undercurl = true }
			)
			vim.api.nvim_set_hl(0, "SpellCap", { fg = ct.blue2, bg = bg, ctermfg = 74, ctermbg = "NONE" })
			vim.api.nvim_set_hl(
				0,
				"SpellRare",
				{ fg = ct.green4, bg = bg, ctermfg = 114, ctermbg = "NONE", sp = ct.green, undercurl = true }
			)
